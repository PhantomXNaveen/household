...
# Aman swami make changes from here
# Chat route, specialy for the Customer support
client = genai.Client(api_key="AIzaSyAaXe4CL1t2vNWj__ZhM5-_a4RoEhBlDzY")
...
def chat():
    ...
    try:
        response = client.models.generate_content(
            model="gemini-2.0-flash", contents="Respond only in plain text string. Do not use special characters like /, *, or symbols. Keep the response under 30 words. Let's start" + user_query
        )
        reply = response.text  # extract the text response
        return jsonify({'reply': reply})
    except Exception as e:
        print(e)
        return jsonify({"reply": "Sorry, I couldn't understand that."}), 500
